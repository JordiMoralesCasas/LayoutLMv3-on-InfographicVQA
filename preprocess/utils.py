...
def clean_text(text):
    """
        Removes all the following characters from the input text: 
            ",", ".", ";", ":", "(", ")", "-", "/", "$", "%", "&", "*" 
    """
    replace_chars = ',.;:()-/$%&*'
...
def get_answer_indices(words, answer):
    """
        Returns the position of a given answer in the text. A harsh find will be used
        if it cannot be easily located.
    """
    answer_tokens = answer.split()
    end_index = None
    start_index = None
